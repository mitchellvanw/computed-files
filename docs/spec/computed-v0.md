...
- Gitignore-aware through the `ignore` crate, configured as the research settled ([ignore semantics](../research/ignore-gitignore-semantics.md)): `.hidden(true)`, `.ignore(false)`, `.git_ignore(true)`, `.parents(true)`, `.require_git(true)`, `.git_exclude(false)`, `.git_global(false)`, `.follow_links(false)`, byte-order sort by file name, the sequential walker. Per-clone and per-user exclude files are off because they would make the snapshot differ between machines. `tree --gitignore` is not git-exact and is not a byte-for-byte reference.
