...
pub fn examine(
    ...
    let args = match Loader::from_opener(&region.opener) {
        ...
        Err(LoadError::Hard(m) | LoadError::Failed { stderr: m }) => {
    ...
    match traced.text {
        ...
        Err(LoadError::Hard(m)) => return error(finding, Verdict::Error, m),
    }
...
fn relevant(ctx: &Ctx, reads: &BTreeSet<PathBuf>) -> Result<BTreeSet<PathBuf>, String> {
    let bound = ctx.bound().map_err(|e| match e {
        LoadError::Hard(m) | LoadError::Failed { stderr: m } => m,
    })?;
...
impl Tracer for Strace {
    fn trace(&mut self, ctx: &Ctx, args: &ExecArgs, region: &str) -> Result<Traced, String> {
        ...
        if !traced {
            let why = match &text {
                Err(LoadError::Failed { stderr } | LoadError::Hard(stderr)) => stderr.clone(),
                Ok(_) => String::new(),
            };
