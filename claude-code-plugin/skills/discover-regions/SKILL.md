...
Find the Markdown, then read it, `.claude/` and `.github/` included. `grep -rl '<!-- computed' --include='*.md' .` names the files that already have regions. Leave those regions alone and look at the prose around them. `computed dupes` lists blocks copied between Markdown files, each with the `file` region that would replace the copy.
...
| A version, or another field of a manifest | That field changes | `value src=Cargo.toml key=package.version` |
