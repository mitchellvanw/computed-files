...
- `inputs=` is comma-separated globset syntax: `**`, `*`, `?`, `[..]`, no braces. Ignore rules do not filter declared inputs. Expansion enters only the directories a leading run of the glob's components can match, so `*.md` reads the region root's listing and nothing below it, and a literal path reads its parent's. A file or directory deleted while expansion lists or reads it is left out, not an error. A glob that matches nothing is a hard error under `run` and under `check`. A matched directory means every file under it. The template file itself is silently excluded from its own snapshot.
